...
{
    // Se gradenow (i==1), scala il timer dell'esame del tempo risparmiato
    if (i && current_ex->time_bef_grade > time(NULL))
    {
        time_t saved = current_ex->time_bef_grade - time(NULL);
        end_time -= saved;
        current_ex->set_time_bef_grade(time(NULL));
    }
